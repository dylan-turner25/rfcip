...
get_col_data <- function(years = c(as.numeric(format(Sys.Date(), "%Y")):as.numeric(format(Sys.Date(), "%Y")) - 4)){
  ...
  variable_years <- c(as.numeric(format(Sys.Date(), "%Y")):as.numeric(format(Sys.Date(), "%Y")) - 4)
  ...
  for(y in years){
    ...
    try({
      ...
      # load the text file
      data <- utils::read.delim(file = list.files(temp_txt,full.names = T),
                                sep = "|", header = F)
    })
    ...
    colnames(data) <- c("commodity_year","state_code","state_abbrv",
  ...
  # load all the `file_to_load` and aggregate them into a single data frame
  col <- files_to_load %>%
