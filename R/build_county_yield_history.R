...
#' - Applies the same type conversion logic as `download_adm` for consistency.
#'
...
build_county_yield_history <- function(years, export_dir = "./data-raw", by_year = TRUE) {
  ...
  # Save as parquet file using compress_adm function (handles type conversion automatically)
  file_name <- paste0(export_dir, "/county_yield_history.parquet")
  compress_adm(df, file_name, metadata_key)
