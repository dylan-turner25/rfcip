...
[![Codecov test coverage](https://codecov.io/gh/dylan-turner25/rfcip/branch/main/graph/badge.svg)](https://codecov.io/gh/dylan-turner25/rfcip?branch=main)
...
## Introduction
`rfcip` allows users to access publicly available data related to the Federal Crop Insurance Program. The package provides a set of functions to easily navigate and access data that is publicly available, but otherwise scattered across different urls and data portals. The package provides a set of functions to access data from the USDA Risk Management Agency's summary of business files, cause of loss files, and price discovery data sets. 
